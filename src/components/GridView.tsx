  ...
  if (restaurants.length === 0) {
    return (
      <div className="flex min-h-[70vh] flex-col items-center justify-center p-8 text-center pt-48">
        ...
        <h3 className="mt-4 text-base font-semibold text-stone-900">No restaurants found</h3>
        ...
        {restaurants.map((restaurant) => {
          ...
          return (
            <div
              ...
              className="group flex flex-col overflow-hidden rounded-2xl border border-[#E6E0D5]/90 bg-[#FFFDFB] shadow-2xs transition-all duration-200 hover:shadow-md hover:border-[#D6CEC0]"
            >
              {/* Image Container */}
              <div
                className="relative h-44 w-full cursor-pointer overflow-hidden bg-[#211C1A]"
              ...
              >
                ...
                />
                <div className="absolute inset-0 bg-gradient-to-t from-[#211C1A]/70 via-transparent to-transparent" />

                {/* Category Badge */}
                <span className={`absolute top-3 left-3 rounded-full px-2.5 py-0.5 text-[10.5px] font-bold shadow-xs backdrop-blur-md border ${meta.badge}`}>
                  {restaurant.category}
                </span>
              ...
              <div className="flex flex-1 flex-col p-4">
                <h3
                  onClick={() => onSelectRestaurant(loc.parentRestaurant, loc.branchId)}
                  className="cursor-pointer font-bold text-[#211C1A] text-base leading-tight hover:text-[#BC5434] transition-colors truncate"
                >
                  {restaurant.name}
                </h3>
                ...
                <div className="mt-3 flex-1">
                  <div className="flex items-center gap-1 text-[11px] font-medium text-[#4A443F]">
                ...
                {/* Card Footer Actions */}
                <div className="mt-4 pt-3 border-t border-[#ECE6DA] flex items-center gap-2">
                  <button
                    onClick={() => onViewOnMap(loc.resolvedRestaurant)}
                    className="flex-1 flex items-center justify-center gap-1.5 rounded-xl border border-[#E2DDD2] bg-[#F4EFE6]/80 py-2 text-xs font-semibold text-[#4A443F] hover:bg-[#EAE4D9] transition-colors cursor-pointer focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-stone-900"
                  >
                  ...
                  <button
                    onClick={() => onSelectRestaurant(loc.parentRestaurant, loc.branchId)}
                    className="flex items-center justify-center rounded-xl bg-[#211C1A] px-3 py-2 text-xs font-semibold text-white hover:bg-[#38312E] transition-colors shadow-2xs cursor-pointer focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-stone-900"
                  >
